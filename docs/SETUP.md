...
- Password policy is configured globally to **10 characters minimum**, requiring **lowercase, uppercase letters, and numbers** in [AppServiceProvider.php](file:///Users/kbmanilla/Desktop/TimeForge/backend/app/Providers/AppServiceProvider.php) (Sprint 57), while database seeders (dev/demo) bypass this check for convenience.
- CORS allowed origins are gated via the `CORS_ALLOWED_ORIGINS` environment variable in `config/cors.php` (Sprint 43), with wildcard configuration blocked in production.
- Exceptions inside `api/*` routes are forced to render JSON via `shouldRenderJsonWhen` in `bootstrap/app.php` (Sprint 43/55), ensuring unauthenticated requests receive a clean 401 JSON response even without an `Accept: application/json` header.
...
- The AI provider decision is resolved to retain the local deterministic stub in production to guarantee 100% data privacy and zero API costs, with future cloud migration guardrails documented in [AI.md](file:///Users/kbmanilla/Desktop/TimeForge/docs/AI.md) (Sprint 58).
